...
> 108 |     await expect(page.getByText('Your account was created successfully. You are now logged in.')).toBeVisible()
      |                                                                                                   ^ Error: expect(locator).toBeVisible() failed
  109 |     await expect(page.getByText('Welcome Testers')).toBeVisible()
  110 |       
  111 | })
  112 | 
  113 | 
  114 | // test('login',async({page,testdataForregistration})=>
  115 | // {
  116 | //     const pageManager = new poManager(page)
  117 | //     const homepage = pageManager.getHomePage()
  118 | //     await homepage.goTo()
  119 | //     const loginpage = pageManager.getLoginPage()
  120 | //     await loginpage.loginToApplication
  121 | //     (
  122 | //         testdataForregistration.username,
  123 | //         testdataForregistration.password
  124 | //     )
  125 | //     // await loginpage.loginButtonClick()
  ...
  131 | // })
  132 | 
  133 | test('fund transfer',async({page,testdataForfundtransfer})=>
  134 | {
  135 |     const pageManager = new poManager(page)
  136 |     const homepage = pageManager.getHomePage()
  137 |     await homepage.goTo()
  138 |     const fundtransferpage = pageManager.getFundTransferPage()
  139 |     await fundtransferpage.clickOnFundTransferLink()
  140 |     await page.waitForTimeout(5000)
  141 |     // The app currently responds with a SOAP/WSDL endpoint for this link, so assert on the URL instead of a UI success message.
  142 |     // await expect(page).toHaveURL(/\/parabank\/services\/ParaBank.*wsdl/)
  143 | 
  144 | }
  145 | )
  146 | })
