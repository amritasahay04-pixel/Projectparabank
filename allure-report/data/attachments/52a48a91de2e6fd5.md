...
- Location: tests\TC.spec.js:143:5
...
Call log:
  - waiting for locator('//a[contains(., \'Log Out\') or contains(., \'Log out\') or contains(., \'logout\')]').first()
...
```ts
  ...
  7  |             this.logoutLink=page.locator("//a[contains(., 'Log Out') or contains(., 'Log out') or contains(., 'logout')]").first()
  8  |        }
  9  | 
  10 |        async goTo()
  11 |        {
